...
def RectangleDetection(img):
    ...
    for object in contours:
        ...
        approx = cv2.approxPolyDP(object, 0.01*perimeter, True) # 最优拟合多边形，用于计算角点数量获取矩形
        CornerNum = len (approx) # 角点数量，用于判断形状

        x, y, w, h = cv2.boundingRect(approx) # x, y, w, h分别为矩形的坐标值和宽度，高度

        if CornerNum == 4 and area >1000:
            return approx



source0 = cv2.VideoCapture("Task3/source1 (online-video-cutter.com).mp4")
source2 = cv2.VideoCapture("Task3/source3.mp4")
...
while source0.isOpened() and source2.isOpened():
    ...
    frameCounter += 1
    if frameCounter == int(source0.get(cv2.CAP_PROP_FRAME_COUNT)-50):
        frameCounter = 0
    ...
    # 由于只因视频为60帧，其他视频为30帧，故只因视频每两帧仅取一帧
    if tt % 2 == 0:
        ret2, imgS2 = source2.read()
    else :
        ret2, imgS2_t = source2.read()
    ret2, imgS2 = source2.read()
    ...
    i = i + 1
    detection = RectangleDetection(imgGray)
    if detection is None:
        continue
    pointsSource0 = np.float32(detection.reshape(4,2))
    pointsSource0R = np.float32(detection.reshape(4,2))
